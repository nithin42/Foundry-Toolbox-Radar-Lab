...
Run `radar.py` against any Toolbox YAML file:

```bash
# Scan with human-readable ASCII table
python tool/radar.py tool/tests/fixtures/risky_toolbox.yaml
```
...
================================================================================
 Total Findings: 7 (HIGH: 3, MEDIUM: 3, LOW: 1)
--------------------------------------------------------------------------------
SEVERITY   | RULE ID   | TOOL NAME          | MESSAGE
--------------------------------------------------------------------------------
[HIGH]     | RULE-01   | delete_database..  | Tool appears to perform mutating actions but does not enforce human approval (require_approval=False).
...
--------------------------------------------------------------------------------
[HIGH]     | RULE-02   | unauthenticated..  | No authentication type configured for tool/connection. Endpoints may be exposed unauthenticated.
  --> Snippet:     authType: 'None'
  --> Remediation: Specify a supported 'authType' ('UserEntraToken', 'AgenticIdentityToken', 'OAuth2', or 'CustomKeys').
--------------------------------------------------------------------------------
[MEDIUM]   | RULE-03   | legacy_erp_conn..  | Tool uses static 'CustomKeys' authentication (API key/PAT). Shared keys lack user attribution and automatic credential rotation.
...
--------------------------------------------------------------------------------
[MEDIUM]   | RULE-04   | user_delegated_..  | Tool uses 'UserEntraToken' passthrough but lacks a specific 'audience' App ID URI.
...
--------------------------------------------------------------------------------
[LOW]      | RULE-06   | broad_cloud_man..  | Wildcard character '*' detected in target, audience, or scope definition.
...
| **`RULE-02`** | **HIGH** | Missing Authentication Type | `authType` is omitted, `null`, or set to `"None"`. |
